 ...
 */
user.prototype.countFilteredRoles = async function ({ search }, context) {
  if ((await checkAuthorization(context, "role", "read")) === true) {
 ...
 */
user.prototype.handleAssociations = async function (
  input,
  benignErrorReporter
) {
  let promises_add = [];
...
async function validForDeletion(id, context) {
  if ((await countAllAssociatedRecords(id, context)) > 0) {
    throw new Error(
      `user with id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`
    );
  }
  return true;
}
...
module.exports = {
   ...
   */
  users: async function ({ search, order, pagination }, context) {
   ...
   * @return {array}             Array of records as grapqhql connections holding conditions specified by search, order and pagination argument
   */
  usersConnection: async function ({ search, order, pagination }, context) {
    if ((await checkAuthorization(context, "user", "read")) === true) {
      helper.checkCursorBasedPaginationArgument(pagination);
      let limit = helper.isNotUndefinedAndNotNull(pagination.first)
        ? pagination.first
      ...
      return await user.readAllCursor(
        search,
        order,
        pagination,
        context.benignErrors
   ...
   */
  readOneUser: async function ({ id }, context) {
    if ((await checkAuthorization(context, "user", "read")) === true) {
   ...
   */
  countUsers: async function ({ search }, context) {
    if ((await checkAuthorization(context, "user", "read")) === true) {
  ...
  vueTableUser: async function (_, context) {
    if ((await checkAuthorization(context, "user", "read")) === true) {
      return helper.vueTable(context.request, user, [
        "id",
        "email",
        "password",
      ]);
  ...
  addUser: async function (input, context) {
    ...
    if (authorization === true) {
      ...
      await createdUser.handleAssociations(
        inputSanitized,
        context.benignErrors
      );
      return createdUser;
  ...
  bulkAddUserCsv: async function (_, context) {
    if ((await checkAuthorization(context, "user", "create")) === true) {
      return user.bulkAddCsv(context, context.benignErrors);
    } else {
      throw new Error("You don't have authorization to perform this action");
    }
  },
   ...
   */
  deleteUser: async function ({ id }, context) {
  ...
  updateUser: async function (input, context) {
    ...
    if (authorization === true) {
      ...
      await updatedUser.handleAssociations(
        inputSanitized,
        context.benignErrors
      );
      return updatedUser;
