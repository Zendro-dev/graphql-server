 ...
 */
role.prototype.countFilteredUsers = async function ({ search }, context) {
  if ((await checkAuthorization(context, "user", "read")) === true) {
 ...
 */
role.prototype.handleAssociations = async function (
  input,
  benignErrorReporter
) {
  let promises_add = [];
...
async function validForDeletion(id, context) {
  if ((await countAllAssociatedRecords(id, context)) > 0) {
    throw new Error(
      `role with id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`
    );
  }
  return true;
}
...
module.exports = {
   ...
   */
  roles: async function ({ search, order, pagination }, context) {
   ...
   * @return {array}             Array of records as grapqhql connections holding conditions specified by search, order and pagination argument
   */
  rolesConnection: async function ({ search, order, pagination }, context) {
    if ((await checkAuthorization(context, "role", "read")) === true) {
      helper.checkCursorBasedPaginationArgument(pagination);
      let limit = helper.isNotUndefinedAndNotNull(pagination.first)
        ? pagination.first
      ...
      return await role.readAllCursor(
        search,
        order,
        pagination,
        context.benignErrors
   ...
   */
  readOneRole: async function ({ id }, context) {
    if ((await checkAuthorization(context, "role", "read")) === true) {
   ...
   */
  countRoles: async function ({ search }, context) {
    if ((await checkAuthorization(context, "role", "read")) === true) {
  ...
  vueTableRole: async function (_, context) {
    if ((await checkAuthorization(context, "role", "read")) === true) {
      return helper.vueTable(context.request, role, [
        "id",
        "name",
        "description",
      ]);
  ...
  addRole: async function (input, context) {
    ...
    if (authorization === true) {
      ...
      await createdRole.handleAssociations(
        inputSanitized,
        context.benignErrors
      );
      return createdRole;
  ...
  bulkAddRoleCsv: async function (_, context) {
    if ((await checkAuthorization(context, "role", "create")) === true) {
      return role.bulkAddCsv(context, context.benignErrors);
    } else {
      throw new Error("You don't have authorization to perform this action");
    }
  },
   ...
   */
  deleteRole: async function ({ id }, context) {
  ...
  updateRole: async function (input, context) {
    ...
    if (authorization === true) {
      ...
      await updatedRole.handleAssociations(
        inputSanitized,
        context.benignErrors
      );
      return updatedRole;
