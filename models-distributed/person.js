...
module.exports = class Person {
    ...
    static adapterForIri(iri) {
      let foundAdapter = registry.filter(adapter => adapters[adapter].recognizeId(
        iri));
    ...
    // Constantin and Asis added the following:

    static assertInputHasId(input) {
      if (! input.id ) {
        // ToDo make sure 'internalPersonId' is always the same as in definition!
    ...
    static addOne(input) {
        ...
        Person.assertInputHasId(input)
        // ToDo input.internalPersonId must be the same as in definition
    ...
    static deleteOne(id) {
        ...
        return responsibleAdapter.deleteOne(input)
    }
