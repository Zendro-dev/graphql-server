...
module.exports.handleRemoteErrors = function (errs, remoteServiceUrl) {
  if (helper.isNonEmptyArray(errs)) {
    return errs.map(function (remoteError) {
      ...
      // web-service
      remoteZendroError.extensions.receivedFrom = helper.isNonEmptyArray(
        remoteZendroError.extensions.receivedFrom
      )
        ? remoteZendroError.extensions.receivedFrom.concat(remoteServiceUrl)
        : [remoteServiceUrl];
...
module.exports.customErrorLog = function (error) {
  if (globals.ERROR_LOG.toUpperCase() === "VERBOSE") {
    ...
    //if not verbose default should be "compact", if for some reason another env was given it should still be compact
    if (error.stack) {
...
/**
 * Class for handling benignErrors which can push benignErrors to the array.
 */
module.exports.BenignErrorArray = class BenignErrorArray extends EventEmitter {
  push(element) {
 ...
 */
module.exports.handleErrorsInGraphQlResponse = function (
  httpResponseData,
  benignErrorReporter
) {
  // check if has errors
  if (helper.isNonEmptyArray(httpResponseData.errors)) {
    benignErrorReporter.push(httpResponseData.errors);
  }
};
 ...
 */
module.exports.handleCaughtErrorAndBenignErrors = function (
  error,
  benignErrorReporter,
  url
) {
  if (!module.exports.isRemoteGraphQlError(error)) {
    ...
    benignErrorReporter.push(
      module.exports.handleRemoteErrors(error.response.data.errors, url)
    );
    throw new Error(
      `Web-service ${url} returned attached (see below) error(s).`
    );
  }
};
