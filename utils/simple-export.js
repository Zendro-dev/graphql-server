...
crateHeaderCSV = function (attributes) {
  ...
  attributes.forEach((att) => {
    str_header += att + `${FIELD_DELIMITER}`;
  });
  str_header = str_header.replace(/.$/, `${RECORD_DELIMITER}`);
...
jsonToCSV = function (row_data, attributes) {
  ...
  attributes.forEach((att) => {
    if (
      row_data[att] === null ||
      row_data[att] === undefined ||
      (Array.isArray(row_data[att]) && row_data[att].length === 0)
    ) {
  ...
  str_csv = str_csv.replace(/.$/, `${RECORD_DELIMITER}`);
  return str_csv;
};
  ...
  try {
    ...
    let getter_resolver =
      inflection.pluralize(
        model_name.slice(0, 1).toLowerCase() +
          model_name.slice(1, model_name.length)
      ) + "Connection";
    ...
    let count_resolver =
      "count" +
      inflection.pluralize(
        model_name.slice(0, 1).toUpperCase() +
          model_name.slice(1, model_name.length)
      );
    ...
    while (hasNextPage) {
      let data = await resolvers[getter_resolver](
        { pagination: batch_step },
        context
      );
      ...
      for await (record of nodes) {
        ...
        if (!writableStream.responseSent) {
          ...
          throw new Error(
            "RESPONSE ALREADY SENT, MOST LIKELY BY TIMEOUT EXCEEDS"
          );
        }
      }
    }
